...
function remSubHostServerSubDomain(subDomainPart, subHost) {
    return subDomainPart && subDomainPart.replace(new RegExp(`^(.*).${subHost}`), '$1')
}

function GetClientIdFromHostname(hostname, opt) {
    var sd = tldjs.getSubdomain(hostname);
    if (Array.isArray(opt.subHost)) {
        opt.subHost.forEach(subHost => {
            sd = remSubHostServerSubDomain(sd, subHost);
        });
    } else if (opt.subHost) {
        sd = remSubHostServerSubDomain(sd, opt.subHost);
    }
    debug(sd);
    return sd;
}
